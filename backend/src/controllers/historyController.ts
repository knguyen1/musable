...
export const trackPlay = asyncHandler(
...
export const getRecentlyPlayed = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const recentHistoryData = await ListenHistoryModel.getRecentlyPlayedSongs(
      userId,
      parseInt(limit as string, 10),
    );
...
export const getMostPlayed = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { limit = 20 } = req.query;
    if (!req.user || typeof req.user.id !== 'number') {
      throw new AppError('Unauthorized', 401);
