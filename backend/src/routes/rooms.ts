...
export default function createRoomRoutes(io: Server) {
  ...
  router.post('/', authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const roomData = {
        ...
        max_listeners: Math.max(
          2,
          Math.min(50, parseInt(max_listeners, 10) || 10),
        ),
      };
  ...
  router.post(
    ...
    async (req: AuthRequest, res) => {
      try {
        ...
        if (Number.isNaN(roomId) || Number.isNaN(songId)) {
  ...
  router.delete(
    ...
    async (req: AuthRequest, res) => {
      try {
        const roomId = parseInt(req.params.id, 10);
        const queueItemId = parseInt(req.params.queueId, 10);

        if (Number.isNaN(roomId) || Number.isNaN(queueItemId)) {
  ...
  router.patch(
    ...
    async (req: AuthRequest, res) => {
      try {
        ...
        if (Number.isNaN(roomId) || Number.isNaN(targetUserId)) {
