...
export class RoomService {
  ...
  private setupSocketHandlers(): void {
    ...
    this.io.on('connection', (socket: AuthenticatedSocket) => {
      ...
      socket.on(
        'room_play',
        async (data: { song_id?: number; position?: number }) => {
      ...
      socket.on(
        'remove_from_queue',
        async (data: { queue_item_id: number }) => {
      ...
      socket.on('disconnect', async () => {
        console.log(
          `🎵 User ${socket.user?.username} disconnected from room service`,
        );
        await this.handleLeaveRoom(socket);
      });
    });
  ...
  private async authenticateSocket(
    ...
    try {
      ...
      console.log(
        '🔍 WebSocket auth debug - Token received:',
        token ? 'YES' : 'NO',
      );
      ...
      console.log('🔍 WebSocket auth debug - Decoded JWT:', {
        id: decoded.id,
      ...
      console.log(
        '🔍 WebSocket auth debug - User found:',
      ...
      console.error(
        '🔍 WebSocket auth debug - Authentication error:',
  ...
  private async handleJoinRoom(
    ...
    try {
      ...
      if (
        participants.length >= room.max_listeners &&
        !participants.some((p) => p.user_id === socket.user?.id)
      ) {
      ...
      console.log(
        `🎵 User ${socket.user.username} joined room ${room.name} (${room.code})`,
      );
    } catch (error) {
  ...
  private async handleLeaveRoom(socket: AuthenticatedSocket): Promise<void> {
    try {
      ...
      const roomId = parseInt(socket.currentRoom.replace('room_', ''), 10);
      ...
      console.log(
        `🎵 After ${socket.user.username} left room ${roomId}, remaining participants: ${remainingParticipants.length}`,
      );
      ...
      if (remainingParticipants.length === 0) {
        ...
        try {
          ...
          console.error(
            `🎵 Failed to auto-delete room ${roomId}:`,
            deleteError,
          );
        }
      } else {
        ...
        console.log(
          `🎵 Notified ${remainingParticipants.length} participants about ${socket.user.username} leaving room ${roomId}`,
        );
      }
  ...
  private async handlePlaybackControl(
    ...
    data: { song_id?: number; position?: number },
  ): Promise<void> {
    try {
      if (!socket.user || !socket.currentRoom) return;
      ...
      if (roomState.is_playing && roomState.play_started_at) {
        const elapsedSeconds =
          (now.getTime() - roomState.play_started_at.getTime()) / 1000;
        roomState.current_position = Math.max(
          0,
          roomState.current_position + elapsedSeconds,
        );
      }
      ...
      console.log(
        `🎵 Room ${roomId} playback control: ${type} by ${socket.user.username}`,
      );
    } catch (error) {
  ...
  private async handleAddToQueue(
    ...
    try {
      ...
      console.log(
        `🎵 Song ${songId} added to room ${roomId} queue by ${socket.user.username}`,
      );
    } catch (error) {
  ...
  private async handleAddToQueueTop(
    socket: AuthenticatedSocket,
    songId: number,
  ): Promise<void> {
    try {
      ...
      await this.handlePlaybackControl(socket, 'song_change', {
        song_id: songId,
      });

      console.log(
        `🎵 Song ${songId} added to TOP of room ${roomId} queue and playing by ${socket.user.username}`,
      );
    } catch (error) {
  ...
  private async handleRemoveFromQueue(
    ...
    try {
      ...
      if (!isHost && !isOwner) {
        socket.emit('room_error', {
          message: 'You can only remove songs you added to the queue',
        });
        return;
      }
      ...
      console.log(
        `🎵 Queue item ${queueItemId} removed from room ${roomId} by ${socket.user.username} (${isHost ? 'host' : 'owner'})`,
      );
    } catch (error) {
