...
export class ListenHistoryModel {
  ...
  async create(data: CreateListenHistoryData): Promise<ListenHistory> {
    ...
    const history = await this.findById(result.lastID as number);
    if (!history) {
  ...
  async getListeningStats(userId?: number): Promise<{
    total_plays: number;
    unique_songs: number;
  ...
  async getMonthlyTrends(): Promise<{
    ...
    total_listening_time: { current: number; previous: number; change: number };
  }> {
  ...
  async getUsersMonthlyTrend(): Promise<{
    current: number;
    previous: number;
    change: number;
  }> {
  ...
  async getSongsMonthlyTrend(): Promise<{
    current: number;
    previous: number;
    change: number;
  }> {
