...
export const getDashboardStats = asyncHandler(
  async (_req: AuthRequest, res: Response) => {
    ...
    const userCount = await UserModel.getAllUsers().then(
      (users) => users.length,
    );
    ...
    const mostPlayedSongs = await ListenHistoryModel.getMostPlayedSongs(
      undefined,
      10,
    );
...
export const getAllInvites = asyncHandler(
  async (_req: AuthRequest, res: Response) => {
...
export const getAllHistory = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (user) {
      const userId = parseInt(user as string, 10);
      history = await ListenHistoryModel.getUserHistory(
        userId,
        parseInt(limit as string, 10),
        parseInt(offset as string, 10),
      );
    } else {
      history = await ListenHistoryModel.getAllHistory(
        parseInt(limit as string, 10),
        parseInt(offset as string, 10),
      );
    }
...
export const getListeningStats = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const trends = await ListenHistoryModel.getListeningTrends(
      user ? parseInt(user as string, 10) : undefined,
    );
    const mostPlayed = await ListenHistoryModel.getMostPlayedSongs(
      user ? parseInt(user as string, 10) : undefined,
    );
...
export const cleanupExpiredInvites = asyncHandler(
  async (_req: AuthRequest, res: Response) => {
...
export const getLibraryPaths = asyncHandler(
  async (_req: AuthRequest, res: Response) => {
...
export const addLibraryPath = asyncHandler(
  async (req: AuthRequest, res: Response) => {
...
export const updateLibraryPath = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
...
export const deleteLibraryPath = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const pathId = parseInt(id, 10);
...
export const getSystemSetting = asyncHandler(
  async (req: AuthRequest, res: Response) => {
...
export const setSystemSetting = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { key } = req.params;
...
export const getAllSystemSettings = asyncHandler(
  async (_req: AuthRequest, res: Response) => {
    // Get all relevant system settings
    const publicSharingEnabled =
      (await SettingsModel.getSetting('public_sharing_enabled')) || 'false';
...
const adminProfilePictureFilter = (
  _req: Request,
...
export const updateUserProfilePicture = asyncHandler(
  async (req: AuthRequest, res: Response) => {
...
export const deleteUserProfilePicture = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { userId } = req.params;
